// src/components/instructor/CourseEdit.jsx
...
import { Image, Upload, X, Save, Plus } from "lucide-react";

const CourseEdit = ({ courseId, onCancel, onCourseUpdated }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    category: "",
    tags: [],
    enrollmentLimit: 0, // New field for limiting enrollments
    requiresApproval: false, // New field for enrollment approval
  });
    ...
    const fetchCourseData = async () => {
      try {
        ...
        setInstructorData({
          ...
          avatar: null,
          avatarPreview: courseData.instructorAvatar || "",
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Handle thumbnail if changed
      let thumbnailUrl = originalThumbnail;
      if (thumbnailFile) {
        const thumbnailData = await uploadImageToCloudinary(thumbnailFile);
        thumbnailUrl = thumbnailData.imageUrl;
      }

      // 2. Handle instructor avatar if changed
      let instructorAvatarUrl = instructorData.avatarPreview;
      if (instructorData.avatar) {
        const avatarData = await uploadImageToCloudinary(instructorData.avatar);
        instructorAvatarUrl = avatarData.imageUrl;
      }
          ...
          <div className="space-y-6">
            ...
            <div className="bg-blue-50 p-4 rounded-md mb-4">
              <p className="text-sm text-blue-800">
                Control how students can enroll in your course with these
                settings.
              </p>
            </div>
